...
--projeto e a data em padrão BR (DD/MM/YYYY),
...
p.proj_nome, ep.data_inicio; -- do funcionário mais velho em projetos dentre os cadastrados SELECT ep.emp_no, e.emp_pri_nome, MIN(DATEDIFF(DAY,ep.data_inicio,GETDATE())) AS funcionario_mais_velho FROM emp_proj ep INNER JOIN empregado e ON e.emp_no = ep.emp_no GROUP BY ep.emp_no, ep.data_inicio, e.emp_pri_nome
...
-- 2) Nome do Departamento, Localização do departamento que não tem
--funcionários cadastrados.
-- A localização deve mostrar apenas as duas primeiras letras,
...
-- 3) Consultar quantos funcionários estão cadastrados em cada projeto
...
-- 4) Consultar quantos projetos não tem empregados associados
